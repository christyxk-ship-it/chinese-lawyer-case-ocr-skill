...
- 已在隔离的 Python 3.12 完整 Paddle 环境中通过依赖检查、13 项测试和 OCRmyPDF/PaddleOCR 真实样本回归。

## v0.4.0

- 案卷目录只保留 `OCR成果/`：最终 PDF、逐页 Markdown 和简明质检报告；中间文件自动清理，模型缓存移到全局目录。
- 自检、空输入、损坏/加密文件、批处理失败和质检失败使用可靠的非零退出码，防止 Agent 误报完成。
- Paddle PDF 增加严格页码校验、已有输出保护、输入输出同源保护和原子写入。
- 安装器改为显式选择宿主；安装前比较，已有 Skill 和依赖环境先备份，失败自动恢复。
- 固定核心依赖版本；Intel Mac 收窄为 OCRmyPDF 基础路线，不再承诺当前官方已停止支持的 PaddlePaddle x86_64 路线。
- 删除公开 Skill 中的个人状态板规则，压缩文档与标准流程；加入单元测试和 GitHub Actions。
